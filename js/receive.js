...
document.addEventListener('DOMContentLoaded', async function () {
	...
	if (loadingOverlay) {
		// Add a small delay to ensure smooth transition and that other scripts have initialized
		setTimeout(() => {
	...
	form.addEventListener('submit', async function (e) {
		...
		try {
            ...
            // 1. 获取信封信息，主要是 passwordHash
            showNotification('正在获取红包信息...', 'info');
            const envelopeInfo = await zEnvelopeContract.getEnvelopeInfo(envelopeId);
            const contractPasswordHash = envelopeInfo.passwordHash;

            // 2. 加密 envelopeId
            const encryptedEnvelopeId = "0x" + goFHE(envelopeId, pkbContent);
